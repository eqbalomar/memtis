...
bpftrace -e "BEGIN {@start = nsecs;} interval:s:1 {
    ...
    printf(\"t=%ld hp_t_lat=%lu.%lu bp_t_lat=%lu.%lu smoothed_walk_completed=%lu smoothed_llc_misses=%lu smoothed_local_llc_misses=%lu smoothed_remote_llc_misses=%lu\n\", (nsecs-@start)/1e9, *kaddr(\"smoothed_t_lat_hp\")/10000, *kaddr(\"smoothed_t_lat_hp\")%10000, *kaddr(\"smoothed_t_lat_bp\")/10000, *kaddr(\"smoothed_t_lat_bp\")%10000, *($addr_walk_completed)*100, *($addr_llc_misses)*100, *($addr_local_llc_misses)*100, *($addr_remote_llc_misses)*100);
}"
