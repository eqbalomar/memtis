...
bpftrace -e "BEGIN {@start = nsecs;} interval:s:1 {
    printf(\"t=%ld local_lat=%lu.%lu remote_lat=%lu.%lu local_occ=%lu remote_occ=%lu local_inserts=%lu remote_inserts=%lu\n\", (nsecs-@start)/1e9, *kaddr(\"smoothed_lat_local\")/10000, *kaddr(\"smoothed_lat_local\")%10000, *kaddr(\"smoothed_lat_remote\")/10000, *kaddr(\"smoothed_lat_remote\")%10000, *($addr_occ_local), *($addr_occ_remote), *($addr_inserts_local), *($addr_inserts_remote));
    printf(\"t=%ld hp_t_lat=%lu.%lu bp_t_lat=%lu.%lu smoothed_walk_completed=%lu smoothed_walk_completed_hp=%lu smoothed_walk_completed_bp=%lu smoothed_llc_misses=%lu smoothed_local_llc_misses=%lu smoothed_remote_llc_misses=%lu\n\", (nsecs-@start)/1e9, *kaddr(\"smoothed_t_lat_hp\")/10000, *kaddr(\"smoothed_t_lat_hp\")%10000, *kaddr(\"smoothed_t_lat_bp\")/10000, *kaddr(\"smoothed_t_lat_bp\")%10000, *($addr_walk_completed)*100, *($addr_walk_completed_hp)*100, *($addr_walk_completed_bp)*100, *($addr_llc_misses)*100, *($addr_local_llc_misses)*100, *($addr_remote_llc_misses)*100);
}"
