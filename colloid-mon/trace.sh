...
addr_walk_completed=$(cat /proc/kallsyms | grep walk_completed | awk '{print "0x"$1}')
addr_all_loads=$(cat /proc/kallsyms | grep all_loads | grep nucleus_mon | awk '{print "0x"$1}')
...
bpftrace -e "BEGIN {@start = nsecs;} interval:s:1 {
    ...
    printf(\"%ld, walk_completed: %lu, all_loads: %lu\n\", (nsecs-@start)/1e9, *($addr_walk_completed), *($addr_all_loads));
}"
