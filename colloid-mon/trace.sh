...
bpftrace -e "BEGIN {@start = nsecs;} interval:s:1 {
    ...
    printf(\"%ld, hp_t_lat: %lu.%lu, bp_t_lat: %lu.%lu, smoothed_walk_completed: %lu, smoothed_all_loads: %lu, walk_completed: %lu, all_loads: %lu\n\", (nsecs-@start)/1e9, *kaddr(\"smoothed_t_lat_hp\")/10000, *kaddr(\"smoothed_t_lat_hp\")%10000, *kaddr(\"smoothed_t_lat_bp\")/10000, *kaddr(\"smoothed_t_lat_bp\")%10000, *($addr_walk_completed)*100, *($addr_all_loads)*100, *($addr_walk_completed_inst)*100, *($addr_all_loads_inst)*100);
}"
