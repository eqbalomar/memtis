...
addr_loads_local=$(cat /proc/kallsyms | grep smoothed_loads_local | grep nucleus_mon | awk '{print "0x"$1}')
addr_loads_remote=$(cat /proc/kallsyms | grep smoothed_loads_remote | grep nucleus_mon | awk '{print "0x"$1}')
addr_all_loads_inst=$(cat /proc/kallsyms | grep all_loads | grep -v smoothed | grep nucleus_mon | awk '{print "0x"$1}')
addr_loads_local_inst=$(cat /proc/kallsyms | grep loads_local | grep -v smoothed | grep -v curr | grep -v prev | grep nucleus_mon | awk '{print "0x"$1}')
addr_loads_remote_inst=$(cat /proc/kallsyms | grep loads_remote | grep -v smoothed | grep -v curr | grep -v prev | grep nucleus_mon | awk '{print "0x"$1}')
...
bpftrace -e "BEGIN {@start = nsecs;} interval:s:1 {
    ...
    printf(\"%ld, hp_t_lat: %lu.%lu, bp_t_lat: %lu.%lu, smoothed_llc_misses: %lu, smoothed_all_loads: %lu, smoothed_loads_local: %lu, smoothed_loads_remote: %lu, llc_misses: %lu, all_loads_diff: %lu, loads_local_diff: %lu, loads_remote_diff: %lu, all_loads: %lu, loads_local: %lu, loads_remote: %lu\n\", (nsecs-@start)/1e9, *kaddr(\"smoothed_t_lat_hp\")/10000, *kaddr(\"smoothed_t_lat_hp\")%10000, *kaddr(\"smoothed_t_lat_bp\")/10000, *kaddr(\"smoothed_t_lat_bp\")%10000, *($addr_walk_completed), *($addr_all_loads), *($addr_loads_local), *($addr_loads_remote), *($addr_walk_completed_inst), *($addr_all_loads_inst), *($addr_loads_local_inst), *($addr_loads_remote_inst), *kaddr(\"nucleus_all_loads\"), *kaddr(\"nucleus_loads_local\"), *kaddr(\"nucleus_loads_remote\"));
}"
