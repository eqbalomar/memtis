...
do
    socket_path="/tmp/redis_$((i-16)).sock"
    taskset -c $i ${MEMTIER_PATH}/memtier_benchmark -S $socket_path --ratio=0:1 --threads=1 --clients=1 --distinct-client-seed -d 384 --key-pattern=R:R --key-maximum=$num_keys --pipeline=128 --hide-histogram -n $num_ops > $OUTPUT_PATH/redis-get-$((i-16)).log 2>&1 &
    pids_memtier_get+=($!)
done
