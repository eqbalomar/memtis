...
do
    socket_path="/tmp/redis_$((i-16)).sock"
    numactl --membind 1 --physcpubind $i,$((i+16)) ${MEMTIER_PATH}/memtier_benchmark -S $socket_path --ratio=0:1 --threads=2 --clients=1 --randomize --distinct-client-seed -d 384 --key-pattern=Z:Z --key-zipf-exp=0.99 --key-minimum=1 --key-maximum=$num_keys --key-prefix="key-" --pipeline=128 --hide-histogram -n $num_ops > $OUTPUT_PATH/redis-get-$((i-16)).log 2>&1 &
    pids_memtier_get+=($!)
done
